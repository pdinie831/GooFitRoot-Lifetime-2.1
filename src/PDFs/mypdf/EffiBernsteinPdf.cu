...
//if(reco==0.0)   printf("EffiBernsteinPdf => mu=%15.15f ret=%15.15f reco=%15.15f gene=%15.15f fTHREADIDX==%d BLOCKIDX==%d\n",mu,ret,reco,gene,THREADIDX,BLOCKIDX);
...
//=================================================================================================================================== 
//
//================ device_MultiCenterEffiBernstein ======================================
// 
//=================================================================================================================================== 
__device__ fptype device_MultiCenterEffiBernstein(fptype *evt, fptype *p, unsigned int *indices) {
//        if ((0 == THREADIDX) && (0 == BLOCKIDX)){
//         printf("==================================================\n");
//        }
// //     struct bernVal bernknvalx;
...
//      }
    if ( (numObservables-2)!=3) {
     printf("device_MultiCenterEffiBernstein error: Max Number of Observables is = 3!!! numObservables = %d\n",numObservables-2);
     return 0.0;
    }
    ...
    fptype gene = (evt[(indices[2 + (indices[0]) + 4])]); // x, y, z...
    if (gene < reco || gene<0.0) {
        printf("device_MultiCenterEffiBernstein error: gene=%f < reco=%f\n",gene,reco);
        return 0;
    }
    fptype nmax = 0;
...
//           mu   +=(p[(indices[ipar])])*bernknintgbinx*bernknintgbiny*bernknintgbinz;
           mu     +=(p[(indices[ipar])])*bernknvalx*bernknvaly*bernknvalz;
//           intg_1 +=(p[(indices[ipar])]);
//           intg_1 +=(p[(indices[ipar])])*bernknintgbinx*bernknintgbiny*bernknintgbinz;
...
__device__ device_function_ptr ptr_to_MultiCenterEffiBernstein = device_MultiCenterEffiBernstein;
...
//     pindices.push_back(dummy     );
     ...
     GET_FUNCTION_ADDR(ptr_to_MultiCenterEffiBernstein);
     initialize(pindices);
 }
